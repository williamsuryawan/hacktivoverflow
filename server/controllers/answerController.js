...
class AnswerController {
  ...
  static editAnswer(req, res, next) {
    ...
    Answer
      ...
      .then(answerToEdit => {
        ...
        if (answerToEdit.author.toString() == req.loggedInUser._id.toString()) {
          ...
          res.status(401).json({
              msg: "you can't edit answers made by other user"
            })
        }
      })
      .catch(err => {
  ...
  static deleteAnswer (req, res, next) {
    ...
    Answer
      .findOne({ _id: req.params.id })
      .then(answer => {
        if (answer.author._id.toString() == req.loggedInUser._id.toString()) {
          Answer
          ...
          res.status(401).json({
              msg: "you can't delete answer made by other"
            })
        }
      })
      .catch(err => {
